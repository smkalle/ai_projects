...
from memory_bridge_agent.tools import (
    evaluate_memory_kit,
    generate_orientation_board,
    load_memory_profile,
)


class MemoryBridgeOrientationBoardTest(unittest.TestCase):
    def setUp(self):
        self.profile = load_memory_profile("examples/memory_profiles/maria_valid.json")

    def test_orientation_board_png_is_created_and_evaluates(self):
        with tempfile.TemporaryDirectory() as tmp:
            path = Path(generate_orientation_board(self.profile, tmp))
            self.assertTrue(path.exists())
            with Image.open(path) as img:
                self.assertEqual(img.size, (1650, 1275))
                self.assertEqual(img.mode, "RGB")

            result = json.loads(evaluate_memory_kit(self.profile, {"orientation_board": str(path)}))
